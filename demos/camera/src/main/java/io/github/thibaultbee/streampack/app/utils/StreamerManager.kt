...
import io.github.thibaultbee.streampack.ext.srt.regulator.srt.SrtRegulatorController
...
class StreamerManager(
    ...
    suspend fun startStream() {
        ...
        val descriptor = when (configuration.endpoint.endpointType) {
            ...
            EndpointType.RTMP -> UriMediaDescriptor(configuration.endpoint.rtmp.url)
        }

        if (configuration.endpoint.endpointType == EndpointType.SRT) {
            if (configuration.endpoint.srt.enableBitrateRegulation) {
                SrtRegulatorController(streamer, bitrateRegulatorConfig = bitrateRegulatorConfig)
            }
        }
