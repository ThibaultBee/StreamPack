...
import io.github.thibaultbee.streampack.ext.rtmp.internal.endpoints.composites.sinks.utils.toRtmpUriMediaDescriptor
...
class RtmpSink(
    ...
    override suspend fun open(mediaDescriptor: MediaDescriptor) =
        open(mediaDescriptor.toRtmpUriMediaDescriptor())

    private suspend fun open(mediaDescriptor: UriMediaDescriptor) {
        require(!isOpened.value) { "SrtEndpoint is already opened" }
        require(mediaDescriptor.sinkType == MediaSinkType.RTMP) { "MediaDescriptor must be a rtmp Uri" }
