...
import com.github.thibaultbee.srtstreamer.mux.ts.packets.Pes
import com.github.thibaultbee.srtstreamer.mux.ts.tables.Pat
import com.github.thibaultbee.srtstreamer.mux.ts.tables.Pmt
import com.github.thibaultbee.srtstreamer.mux.ts.tables.Sdt
import com.github.thibaultbee.srtstreamer.mux.ts.utils.MuxConst
...
class TSMux(
    ...
    private fun retransmitPsi(forcePat: Boolean) {
        sdt.packetCount += 1
        if (sdt.packetCount == MuxConst.SDT_PACKET_PERIOD) {
            sendSdt()
        }

        pat.packetCount += 1
        if ((pat.packetCount == MuxConst.PAT_PACKET_PERIOD) || forcePat) {
            sendPat()
            sendPmts()
        }
    }
    ...
    private fun addStreams(service: Service, streamsMimeType: List<String>): List<Short> {
        ...
        } ?: Pmt(muxListener, service, service.streams, getNewPid())
