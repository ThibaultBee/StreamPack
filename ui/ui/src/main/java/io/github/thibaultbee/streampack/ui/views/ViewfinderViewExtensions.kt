...
fun ViewfinderView.bindToStreamerSource(
    ...
    // Forward View Gestures to Camera Settings
    onZoomRatioChanged = { scale, pinchCenter ->
        val source = streamer.videoInput.sourceFlow.value
        if (source is IPreviewControllableSource) {
            lifecycleOwner.lifecycleScope.launch {
                source.setZoomOnPinch(scale, pinchCenter)
            }
        }
    }

    onTapToFocus = { point, _ ->
        val source = streamer.videoInput.sourceFlow.value
        if (source is IPreviewControllableSource) {
            lifecycleOwner.lifecycleScope.launch {
                source.setTapToFocus(
                    ...
                    this@bindToStreamerSource.autoCancelDurationMillis
                )
            }
        }
    }
    ...
    val job = lifecycleOwner.lifecycleScope.launch {
        ...
        try {
            streamer.videoInput.sourceFlow.collect { source ->
                ...
                val controllableSource = source as? IPreviewControllableSource
                this@bindToStreamerSource.isPinchToZoomEnabled =
                    controllableSource?.isPinchToZoomSupported == true && isPinchToZoomEnabled
                this@bindToStreamerSource.isTapToFocusEnabled =
                    controllableSource?.isTapToFocusSupported == true && isTapToFocusEnabled

                controllableSource?.setOnZoomListener(onZoomChanged)
                        ...
                        combine(
                            ...
                            .collect { previewSize ->
                                try {
                                    ...
                                    val surface = requestSurface(
                                        previewSize,
                                        previewableSource.previewConfiguration
                                    )
            ...
            previewJob?.cancel()
            (streamer.videoInput.sourceFlow.value as? IPreviewControllableSource)?.setOnZoomListener(
                null
            )
            this@bindToStreamerSource.isPinchToZoomEnabled = false
            this@bindToStreamerSource.isTapToFocusEnabled = false
...
}.filter { it.width > 0 && it.height > 0 }.distinctUntilChanged()
