...
class TrackFragmentHeaderBox(
    ...
    override fun write(buffer: ByteBuffer) {
        super.write(buffer)
        buffer.putInt(id)
        baseDataOffset?.let { buffer.putLong(it) }
        sampleDescriptionIndex?.let { buffer.putInt(it) }
        defaultSampleDuration?.let { buffer.putInt(it) }
        defaultSampleSize?.let { buffer.putInt(it) }
        defaultSampleFlags?.let { buffer.putInt(it) }
    }
